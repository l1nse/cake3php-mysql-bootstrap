...
class CotizacionesController extends AppController
{
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            ...
            $get_data['entidade_id'] = $entidade_id[0]->id;

            //$rs_cotizacion  = $this->Cotizaciones->
            $max_cotizaciones = $this->Cotizaciones->find('all')->order(['cotizacione_id' => 'DESC'])->limit(1)->toArray();

            //var_dump($max_cotizaciones[0]); die;
            if(is_array($max_cotizaciones) && count($max_cotizaciones)>0){
                //echo 'uno'; die;
                $get_data['folio'] = 1;
            }else{
                if(isset($max_cotizaciones[0]->cotizacione_id) && $max_cotizaciones[0]->cotizacione_id !=''){
                    //echo 'dos'; die;
                    $get_data['folio']  = ($max_cotizaciones[0]->cotizacione_id+1);
                }else{
                    //echo 'tres'; die;
                    $get_data['folio'] = 1;
                }
            ...
            //echo ($get_data['cotizacione_id']); die;

            $get_data['active'] = 1;
            $get_data['user_id']  = $this->_getUser();

            //var_dump($max_cotizaciones); die;

            //var_dump($entidade_id[0]->id); die;
